    ...
    if not instance.is_processed:
        ...
        if convert.sucess:
            ...
            if UserSettings.objects.filter(
                ...
                workflow_tool = UserSettings.objects.filter(
                    user=instance.record_creator.pk).first(
                ).workflow_tool
            if auto_qc != "None" and instance.factor_8_name != "study_group":
            ...
            if workflow_tool != "None" and \
                ...
                # end with last
                worfkflow_setting_list = workflow_tool.split("qc")
                # list item 1 is qc app pk, 2 is qc preset number
                process_app = ProcessingApp.objects.filter(
                    pk=worfkflow_setting_list[0]).first()
                newqueue = {
                    "processing_name": instance.pk,
                    'processing_app': process_app,
                    'process_creator': instance.record_creator,
                ...
                for item in SampleRecord.objects.filter(
                        ...
                        factor_8_value=instance.factor_8_value[:-4]):
                ...
                preset_file = os.path.join(
                    settings.MEDIA_ROOT, getattr(
                        process_app,
                        f"preset_{worfkflow_setting_list[1]}").name)
                with zipfile.ZipFile(preset_file, 'r') as z:
                    for target_file in z.namelist():
                        for n in range(1, 4):
                            if target_file.startswith(f'input_file_{n}'):
                ...
                newtask.save()
                logger.info(
                    f"Batch process {newtask.pk} was created "
...
class UserSettings(models.Model):
    ...
    workflow_tool = models.TextField(
        max_length=100, blank=True, null=True, default="None")
...
def move_file(sender, instance, **kwargs):
    """_process the uploaded files to year/month/date folders_
    """
    update_url = {}
